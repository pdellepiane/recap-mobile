...
import { mockHostsLineFromHomeItem } from '@/src/features/home/data/eventDetailRemoteMap';
...
export function mapHomeEventApiItemToDomain(remote: HomeEventItem): Event {
  const locationParts = [remote.location?.trim(), remote.city?.trim()].filter((p): p is string =>
